...
def getjsondata():
    tilt_angle=35#Taken as User input in degrees
    azimuth=[90] #Taken as User input in degrees
    interval=30 #User input in minutes and shows option for only 30 or 15 mins 
    latitude=-33.93
    global_irr=[6.61,5.76,5,3.9,2.81,2.37,2.64,3.69,4.79,5.61,6.19,6.74] #Value from Bom data
    ...
    for k in range(len(azimuth)):
        for j in range(len(ax)):
            ...
            if(j==0):
                ...
                value2=np.degrees(np.arccos(-1*np.arctan(np.radians(latitude+tilt_angle))*np.arctan(np.radians(declination_angle))))
                if (value1 < value2):
                    sunset_angle=value1
                else:
                    sunset_angle=value2
                ...
                for i in range(len(hourofday)): 
                    ...
                    if(hour_angle[i]==0 and hourofday[i]!=12):
                        ...
                        theta_t[i]=np.degrees(np.arccos(np.sin(np.radians(declination_angle))*np.sin(np.radians(latitude))*np.cos(np.radians(tilt_angle))-np.sin(np.radians(declination_angle))*np.cos(np.radians(latitude))*np.sin(np.radians(tilt_angle))*np.cos(np.radians(azimuth[k]))+np.cos(np.radians(declination_angle))*np.cos(np.radians(latitude))*np.cos(np.radians(tilt_angle))*np.cos(np.radians(hour_angle[i]))+np.cos(np.radians(declination_angle))*np.sin(np.radians(latitude))*np.sin(np.radians(tilt_angle))*np.cos(np.radians(azimuth[k]))*np.cos(np.radians(hour_angle[i]))+np.cos(np.radians(declination_angle))*np.sin(np.radians(tilt_angle))*np.sin(np.radians(azimuth[k]))*np.sin(np.radians(hour_angle[i]))))
                        if(theta_t[i]>90):
                            rb[i]=0
                        else:
                        ...
                        interval_diff[i]=interval_diff[i]*((1+np.cos(np.radians(tilt_angle)))/2)
        ...
        #output
        filename="output_tilt="+str(abs(tilt_angle))+"_azimuth="+str(azimuth[k])+".xlsx"
        #output.to_excel(filename,index=False)
        #jsondata = output.to_json(orient='split')
        return output
